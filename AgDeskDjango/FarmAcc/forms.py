...
class JoinFarmForm(forms.Form):
    ...
    linking_code = forms.CharField(label="Linking Code:", widget=forms.TextInput(attrs={'class': 'form-control'}))
...
class NewFarm(ModelForm):
    ...
    class Meta:
        model = FarmInfo
        fields = ['farm_name', 'farm_street', 'farm_state', 'farm_postcode', 'farm_bio', 'farm_image']

# - - - - - - - - - - - - - - - - - - - - - - - - - - -
"""
The following section defines the forms which are used to upload documents with the Django application.
"""

class UploadDocument(ModelForm):
    """
    The UploadDocument form is used for uploading files within the 'fileView' and 'editFile' views. It defines the 
    required attributes (which need to be filled in) when uploading files to the application.
    """
